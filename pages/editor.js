...
/*
  ...
  - Up to 4 source slots (pattern + zoom + bounds)
  - Load tiles into memory as dataURLs (persist across source changes)
  - Place, pick, swap, duplicate, erase on an editor grid
  - Grid up to 32x32 tiles (8192px at 256px/tile)
  - Download final PNG
*/

var MAX_GRID = 32; // 32*256 = 8192
...
function emptySource() {
  return { name: "", pattern: "", zoom: 3, minX: 0, maxX: 7, minY: 0, maxY: 7, tileSize: 256, color: COLORS[0] };
}
...
export default function Editor() {
  ...
  // Tile bank: key -> dataURL (persists across source changes)
  var bankRef = useRef({});
  ...
  // grid cells: array of {key, dataUrl, srcIdx, ox, oy} or null
  ...
  // Selected tile for placing (from palette)
  a = s(null); var selected = a[0], setSelected = a[1];

  // Swap: first cell index
  a = s(null); var swapFirst = a[0], setSwapFirst = a[1];

  // Palette filter
  a = s(-1); var paletteFilter = a[0], setPaletteFilter = a[1]; // -1 = all

  // Palette tiles (derived from bank)
  a = s([]); var paletteTiles = a[0], setPaletteTiles = a[1];

  // Grid zoom for mobile
  ...
  // Auto-detect from pasted tile URL
  async function detectSource() {
    ...
    try {
      ...
      if (d.success) {
        ...
        setLoadStatus("Detected! " + cols + "x" + rows + " tiles at zoom " + d.detectedZoom);
      } else {
  ...
  // Load tiles from a source into bank
  async function loadSource(idx) {
    ...
    for (var y = src.minY; y <= src.maxY; y++) {
      for (var x = src.minX; x <= src.maxX; x++) {
        ...
        var rawUrl = buildUrl(src.pattern, src.zoom, x, y);
  ...
  // Auto-place all tiles from a source into the grid
  function autoPlace(idx) {
    ...
    // Build new grid with resize + placement in one pass
    setGrid(function(old) {
      var oldW = gridW;
      ...
      for (var dy = 0; dy < rows; dy++) {
        for (var dx = 0; dx < cols; dx++) {
          ...
          if (bank[key]) {
            var gi = dy * newW + dx;
            if (gi < ng.length) {
              ng[gi] = { key: key, dataUrl: bank[key], srcIdx: idx, ox: src.minX + dx, oy: src.minY + dy };
            }
          }
        }
      }
  ...
  function downloadGrid() {
    ...
    var H = gridH * tSize;
    if (W > 8192 || H > 8192) {
      setDlStatus("Canvas exceeds 8192px! Reduce grid size.");
      return;
    }
    ...
    for (var i = 0; i < grid.length; i++) {
      ...
      (function(idx, cell) {
        ...
        var p = new Promise(function(res) {
          var img = new Image();
          img.onload = function() {
            ctx.drawImage(img, gx * tSize, gy * tSize, tSize, tSize);
            loaded++;
            res();
          };
  ...
  return (
    <div style={S.wrap}>
      ...
      <p style={S.sub}>Load tiles from multiple sources, compose and download</p>
        ...
        {sources[activeSrc] && (function() {
          ...
          return <>
            ...
            <input value={src.pattern} onChange={function(e){updateSource(idx, "pattern", e.target.value)}}
              placeholder="https://.../{z}/{x}/{y}.webp" style={S.input} />
            <div style={S.g5}>
            ...
            <p style={{fontSize: "0.68rem", color: "#666", marginTop: 4}}>{cols + "x" + rows + " = " + (cols * rows) + " tiles"}</p>
      ...
      <div style={Object.assign({}, S.card, {borderColor: "#222"})}>
        ...
        <p style={{fontSize: "0.72rem", color: "#999", lineHeight: 1.7}}>
          ...
          {"2. Paste a tile URL > Detect to auto-fill pattern and bounds"}<br/>
          {"3. Load Tiles to download them into the palette"}<br/>
          {"4. Auto-Place fills the grid, or use Place tool manually"}<br/>
          {"5. Pick = grab a tile from grid to re-place elsewhere (duplicate)"}<br/>
          {"6. Swap = tap two cells to exchange them"}<br/>
          {"7. Erase = clear a cell"}<br/>
          {"8. Download PNG when done (max 8192x8192px)"}
        </p>
      </div>
    </div>
  );
...
var S = {
  ...
  si: { width: "100%", padding: 7, background: "#0d0d14", border: "1px solid #333", borderRadius: 6, color: "#eee", fontSize: "0.75rem", boxSizing: "border-box" },
  ...
  mono: { background: "#0d0d14", border: "1px solid #2a2a3a", borderRadius: 5, padding: 6, fontFamily: "monospace", fontSize: "0.62rem", color: "#6ee7b7", wordBreak: "break-all" },
};
